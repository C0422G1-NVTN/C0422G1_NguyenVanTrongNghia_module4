...
public class LibraryAspect {
    ...
    @Before(value =
            "execution(* com.example.library.controller.LibraryController.*(..))")
    ...
    @AfterReturning(pointcut = "execution(* com.example.library.controller.LibraryController.rentBook(..))")
    public void logChangeBookForRent(JoinPoint joinPoint) {
        String methodName = joinPoint.getSignature().getName();
        System.err.println("Bạn đã vào " + methodName + " và thay đổi trạng thái sách");
    }

    @AfterReturning(pointcut = "execution(* com.example.library.controller.LibraryController.payBook(..))")
